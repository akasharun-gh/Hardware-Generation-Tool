...
int main(int argc, char* argv[]) {
   ...
   if ((mode == 1) && (argc == 6)) {
      ...
      // We will account for overflow in the calculation of expected outputs
      vector<long> fVector;
      genRandomVector(fVector, M, T/2);
      ...
      genRandomVector(fVector1, M1, T/2);
      genRandomVector(fVector2, M2, T/2);
      genRandomVector(fVector3, M3, T/2);
...
void computeOutputs(vector<long>& f, vector<long>& x, vector<long>& y, int N, int M, int T) {
   ...
   for (int i=0; i<iterations; i++) {
      for (int n=0; n<L; n++) {
         y.push_back(0);
         for (int k=0; k<M; k++) {
            long prod = checkOverflow(f[k]*x[n+k+i*N], T);
            y[n+i*L] = checkOverflow(y[n+i*L] + prod, T);
         }
      }
   }
}

// Check to see if v will overflow when represented with given number of bits
// If the operation overflows, return the overflowed result (consistent with what)
// your hardware would return.
long checkOverflow(long v, int bits) {
